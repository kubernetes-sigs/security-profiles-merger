...
// object, members the target type has no field for, members that name a
// field only ignoring case, bytes that are not valid UTF-8, and data behind
// the document. The command and the three profile packages share it, so
// that a library caller can refuse the same documents the command refuses.
package strictjson
