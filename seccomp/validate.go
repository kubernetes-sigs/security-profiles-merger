...
// SCMP_ACT_NOTIFY appears only where runc loads it, which is what a runtime
// needs to load the profile at all. Intersect and Union run it on every
...
// action") and on the write syscall, which the listener needs to answer a
// notification. libseccomp itself accepts a NOTIFY default, so this is a runc
// constraint, and a profile that hits it fails container creation rather than
// producing a weaker filter.
func validateNotifySupport(profile *specs.LinuxSeccomp) error {
	...
	for idx := range profile.Syscalls {
		...
		if entry.Action == specs.ActNotify && slices.Contains(entry.Names, notifyWriteSyscall) {
			errs = append(errs, fmt.Errorf(
